// backend/scripts/downloadBible.js
const fs = require('fs');
...
async function downloadBible() {
  ...
  for (const book of books) {
    ...
    const totalChapters = chaptersPerBook[book] || 1;
        ...
        if (response.ok) {
          ...
          if (data.verses) {
            ...
            data.verses.forEach(v => {
              bible[book][ch][v.verse] = v.text.trim();
            });
          }
        }
        console.log(`  ✅ ${book} ${ch}/${totalChapters}`);
      } catch (e) {
        console.log(`  ❌ ${book} ${ch} failed`);
      }
  ...
  const outputPath = path.join(__dirname, '..', 'data', 'bible-kjv.json');
  fs.writeFileSync(outputPath, JSON.stringify(bible));
  console.log(`\n✅ Bible saved to ${outputPath}`);
